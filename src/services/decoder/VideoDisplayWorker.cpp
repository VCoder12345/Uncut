#include "VideoDisplayWorker.h"
VideoDisplayWorker::VideoDisplayWorker(VideoStreamBuffer *vbuffer,
                                       SDL_AudioStream *stream,
                                       AudioDecodeWorker *audioDec)
    : vbuffer(vbuffer), stream(stream), audioDec(audioDec) {}

void VideoDisplayWorker::interrupt() { interrupted.store(true); }

void VideoDisplayWorker::startDisplaying() {
  ...
  while (!interrupted.load() && !vbuffer->isFinished()) {
    ...
    while (!interrupted.load() && frame &&
           frame->pts < audioPts - 0.03) { // 30ms late? Drop it.
      // qDebug() << "Dropping late frame at " << frame->pts << " (audio: " <<
      // audioPts << ")";
    ...
    if (frame->pts <= audioPts + 0.005) { // Allow small lead
      // qDebug() << "Displaying frame " << frame->pts << " (audio: " <<
      // audioPts << ")";
